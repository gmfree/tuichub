...
rarecurve(otu_table(ASV_physeq), step=50, cex=0.5, label=FALSE)
abline(v = 5000, lty = "dotted", col="red", lwd=2)
dev.off() 

# remove samples with < 5000 reads
ASV_physeq_prune <- prune_samples(sample_sums(ASV_physeq) > 5000, ASV_physeq) #89 samples
rarecurve(otu_table(ASV_physeq_prune), step=50, cex=0.5, label=FALSE)
